...
モード別オプション:
- **tweet**: `-n, --num-variants <number>` — バリアント数、`--style-hint <hint>`
- **converse**: `--turns <number>` — ターン数（未指定で自動判断）、`--auto-limit <number>` — 自動判断時の上限（デフォルト: 8）、`--no-eval`
- **multi**: `--topic <topic>` — 会話トピック、`--turns <number>`
- **chat**: `--eval` — 評価を有効化
...
- このディレクトリにはツイートの内容が含まれるため、`.gitignore` でデフォルト除外されています
