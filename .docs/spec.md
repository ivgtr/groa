...
5. `groa generate alice "AIの未来"` でテキスト生成（Step 6-8）
...
| 7 | テキスト生成 | PersonaDocument, TaggedTweet[] (few-shot), トピック | GeneratedText | standard + Prompt Caching | ~$0.009/件 |
| 8 | 品質評価 | GeneratedText, TaggedTweet[] (比較用), PersonaDocument | EvaluationResult | standard | ~$0.01/件 |
...
groa generate <name> <topic> [--n 5] [--temp 0.7]      # Step 6-8: 生成
```
