...
interface RequestOptions {
  ...
  useCache: boolean;      // Prompt Caching使用の有無（anthropicのみ有効）
  useBatch: boolean;      // Batch API使用の有無（anthropicのみ有効）
}
...
### 3.4 OpenRouterバックエンド (`openrouter`)

OpenRouter の OpenAI 互換 Chat Completions API (`https://openrouter.ai/api/v1/chat/completions`) に `fetch` でHTTPリクエストを発行する実装。

#### リクエスト形式

OpenAI 互換の `messages` 配列形式を使用する。Anthropic の `system` フィールド分離は行わず、`system` ロールを `messages` 配列に含める。

```typescript
{
  model: string;          // OpenRouter モデルID（例: "google/gemini-2.5-flash-lite"）
  messages: Message[];    // system / user / assistant を含む
  max_tokens: number;
  temperature?: number;
}
```
...
`Authorization: Bearer <apiKey>` ヘッダーを使用する（Anthropic の `x-api-key` とは異なる）。

#### 非対応機能

- **Prompt Caching**: `useCache` は無視される。`cachedTokens` は常に `0`
- **Batch API**: `useBatch` は無視される

#### リトライ・タイムアウト

Anthropic バックエンドと同じ仕様（`withRetry` 共有、120秒タイムアウト）。

### 3.5 Claude Code CLIバックエンド (`claude-code`)

`child_process.spawn` で `claude -p` をストリーミングモードで実行し、プロンプトをstdin経由で送信、stdoutからNDJSONイベントをストリーミングで読み取る実装。
...
// readline で NDJSON を行単位パース
const rl = createInterface({ input: child.stdout });
rl.on("line", (line) => {
  const event = JSON.parse(line);
  if (event.type === "result") { /* 最終結果 */ }
});
// アイドルタイムアウト: 最後のイベントから180秒
```

> `--verbose` は `--output-format stream-json` の使用に必須。

#### `--output-format stream-json` のイベント構造

NDJSON（1行1イベント）で以下のイベントが流れる:

```
{"type":"system","subtype":"init",...}           // 初期化
{"type":"assistant","message":{...}}             // 応答チャンク（複数回）
{"type":"result","subtype":"success",...}         // 最終結果
```
...
- **リトライ**: エラー時、最大3回リトライ（`withRetry` 共有）
- **タイムアウト**: アイドルタイムアウト180秒。最後のストリーミングイベントからの経過時間で判定するため、応答生成中であれば無制限に待つ。固定タイムアウトは使用しない
- **コスト追跡**: `result` イベントの `total_cost_usd` フィールドから取得。取得不能な場合は `costUsd: null` とする
...
- **`anthropic` バックエンド**: Batch APIで一括投入。50%割引が適用される
